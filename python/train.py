...
parser.add_argument("-e", "--max-epochs", type=int,
                    help="Maximum epochs count", default=500)
...
parser.add_argument("--scheduler-step-size",
                    help="Scheduler step size", type=int, default=5)
...
def save_model(epoch):
  ...
  print(
      f">> Saved checkpoint '{checkpoint_dir}/checkpoint-{epoch}.zip' in {round(time.time() - start_saving, 3)}s")
...
for epoch in range(initial_epoch, initial_epoch + max_epochs + 1):
  ...
  for i, batch in enumerate(train_data):
    ...
    targets = torch.tensor(targets).t().to(device)
    ...
    loss = criterion(output.transpose(
        0, 1).reshape(-1, ntokens), targets.t().view(-1))
    loss.backward()
